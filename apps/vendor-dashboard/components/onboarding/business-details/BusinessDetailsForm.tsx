...
export function BusinessDetailsForm({ application, countries, hasDocuments }: Props) {
  ...
  // If the user has already uploaded documents, changing country or vendor type
  // will delete them (requirements differ per country/type). Show a warning first.
  ...
  function handleDialogConfirm() {
    // setTimeout(0) defers the setValue until after the dialog's close animation
    // completes, preventing a React batching conflict with onOpenChange.
    setTimeout(() => {
  ...
  useEffect(() => {
    ...
    async function fetchVendorTypes() {
      try {
        // countryId comes from a controlled SearchableSelect populated with
        // server-fetched UUIDs — not user-typed — so no additional encoding needed.
        ...
        // Re-select the saved vendor type if it's valid for this country
  ...
  // ── Show "Other" text input when vendor type is "Other" ───────────────────
  useEffect(() => {
  ...
  async function onSubmit(values: BusinessDetailsFormData) {
    ...
    try {
      ...
      const res = await fetch('/api/onboarding/application', {
        method: 'POST',
        // Content-Type is required — without it, Express body-parser won't
        // parse the JSON body and the route handler receives an empty object.
    ...
    <>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <FormCard icon={<Building2 className="h-4 w-4" />} title="Business Registration">
            <div className="grid grid-cols-1 gap-x-5 gap-y-5 sm:grid-cols-2 xl:grid-cols-3">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Tax ID / KRA PIN</FormLabel>
                    <FormControl><Input placeholder="Optional" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
          ...
          {/* ── Business Contact ───────────────────────────────────────── */}
          <FormCard icon={<Phone className="h-4 w-4" />} title="Business Contact">
          ...
          {/* ── Owner Information ──────────────────────────────────────── */}
          <FormCard icon={<User className="h-4 w-4" />} title="Owner Information">
          ...
          {/* ── Business Address ───────────────────────────────────────── */}
          <FormCard icon={<MapPin className="h-4 w-4" />} title="Business Address">
          ...
          <div className="flex justify-end pb-8 pt-2">
            <Button type="submit" disabled={isSubmitting} size="lg" className="min-w-40">
              {isSubmitting ? (
...
function FormCard({
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
      <div className="flex items-center gap-2.5 border-b border-border bg-muted/30 px-5 py-4">
