...
/** PATCH /api/food-tags/[kind]/[tagRef] — rename / re-describe. GLOBAL scope. */
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ kind: string; tagRef: string }> }) {
  ...
  try {
    ...
    const res = await fetch(`${BACKEND}/admin/v1/food-tags/${kind}/${tagRef}`, {
      method : "PATCH",
    ...
    console.error("[food-tags-update]", err)
